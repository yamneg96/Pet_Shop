...
import { signIn } from "next-auth/react"; import { motion } from "framer-motion"; import { PawPrint, Chrome } from "lucide-react"; import Link from "next/link";

export default function SignInPage() { 
  const containerVariants = { 
    hidden: { opacity: 0, scale: 0.95 }, 
    visible: { opacity: 1, scale: 1, transition: { duration: 0.5, staggerChildren: 0.1, ease: "easeOut" } } 
  };
  ...
  return ( <div className="min-h-[85vh] flex items-center justify-center px-6 relative overflow-hidden"> {/* Background Decorative Blurs */} <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-orange-500/10 rounded-full blur-[100px] -z-10" /> <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-primary/5 rounded-full blur-[100px] -z-10" />
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex justify-center mb-6">
        <div className="bg-orange-500 p-4 rounded-2xl text-white shadow-lg shadow-orange-500/20 rotate-3 hover:rotate-0 transition-transform duration-300">
          <PawPrint size={32} />
        </div>
      </motion.div>
